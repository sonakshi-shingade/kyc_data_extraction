...
if st.session_state.logged_in:
    menu = st.sidebar.selectbox("Select Option", ["Upload KYC", "View Records", "View Details", "Logout"])
else:
...
if menu == "Register":
    ...
    if st.button("Register"):
        ...
        st.success(res.json()["message"]) if res.ok else st.error(res.json()["message"])
...
# Upload KYC
elif menu == "Upload KYC":
    if st.session_state.logged_in:
        st.subheader("📤 Upload Scanned KYC Document")
        uploaded_file = st.file_uploader("Upload PNG/JPG file", type=["png", "jpg", "jpeg"])

        if uploaded_file and st.button("Submit KYC"):
            file_bytes = uploaded_file.read()
            b64_str = base64.b64encode(file_bytes).decode()
            image_data = base64.b64decode(b64_str)  # Remove the 'data:image/png;base64,' part
            image = Image.open(io.BytesIO(image_data)) 
            st.image(image)           
            mime = uploaded_file.type
            data_url = f"data:{mime};base64,{b64_str}"

            payload = {"username": st.session_state.username, "image_b64": data_url}
            headers = {"Authorization": f"Bearer {st.session_state.token}"} if st.session_state.token else {}
            res = requests.post(f"{API_URL}/upload-kyc", json=payload, headers=headers)
            res_details = requests.post(f"{API_URL}/extract-text-from-kyc", json=payload, headers=headers)

            if res.ok and res_details.ok:
                st.markdown(f"DOB:{res_details.json().get("DOB")}")
                st.markdown(f"PAN:{res_details.json().get("PAN")}")                
                st.success(f"KYC uploaded! Record ID: {res.json().get('record_id')}")
            else:
...
# View KYC records
    ...
    if st.session_state.logged_in:
        ...
        if st.button("Get Records"):
            res = requests.get(f"{API_URL}/kyc-records/{username}", headers=headers)
            if res.ok:
                records = res.json()
                if records:
                    for r in records:
                        st.write(f"ID: {r['id']} | Name: {r['name']} | Document: {r['doc_type']}")
                else:
...
# View KYC details
    ...
    if st.session_state.logged_in:
        ...
        if st.button("Get Details"):
            res = requests.get(f"{API_URL}/kyc-details/{record_id}", headers=headers)
            if res.ok:
                st.json(res.json())
            else:
